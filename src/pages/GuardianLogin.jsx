...
export default function GuardianLogin() {
    ...
    const validateForm = () => {
        const newErrors = {};
        if (isSignUp && !formData.email.includes('@guardian.')) {
            newErrors.email = 'Guardian email must contain @guardian. domain';
        }
    ...
    return (
        <div className="login-wrapper">
            <div className="login-card" onKeyPress={handleKeyPress}>
                ...
                <input type="email" placeholder="Guardian Email (@guardian.domain)" value={formData.email} onChange={(e) => handleInputChange('email', e.target.value)} onKeyPress={handleKeyPress} className={errors.email ? 'error' : ''} />
